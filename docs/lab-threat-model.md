...
| Runner and Ollama adapter | Runs paired trials using trusted candidate settings and treats evidence/model output as untrusted | Private CLI implemented with reference binding, randomized order, model-digest preflight, private endpoint allowlist, no redirects, total per-call deadlines, all-string Zeek isolation with recognized field names, optional schema-enforced assessment responses, non-free-form candidate reasoning, strict response validation, and canary redaction (`triagewall/lab_runner.py`, `triagewall/zeek_isolation.py`, `scripts/run_lab_experiment.py`) |
