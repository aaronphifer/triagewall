#!/usr/bin/env python3
"""Create trusted baseline/candidate/specification files for Lab experiment 1."""
...
CANDIDATE_INSTRUCTION = (
    "End the reasoning with one sentence beginning exactly 'Zeek assessment:'. "
    "Cite concrete facts observed in the supplied Zeek JSON that affected the "
    "verdict. If Zeek only corroborated the flow and did not change the verdict, "
    "state that explicitly. Attribute only facts present in the supplied Zeek "
    "JSON; a matched flow alone does not establish maliciousness."
)
...
def _arguments(argv=None):
    parser = argparse.ArgumentParser(
        description="Build trusted prompt-only inputs for TriageWall Lab experiment 1."
    )
    ...
    parser.add_argument("--baseline-id", default="zeek-exp1-core-baseline")
    parser.add_argument("--candidate-id", default="zeek-exp1-explicit-assessment")
    parser.add_argument("--experiment-id", default="zeek-explicit-assessment-001")
...
def main(argv=None):
    ...
    try:
        ...
        print(f"Could not build Lab experiment 1 safely: {message}", file=sys.stderr)
        return 1
    print(f"Created trusted Lab experiment inputs in {args.output_dir.resolve()}")
    return 0
